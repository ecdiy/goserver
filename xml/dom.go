...
func (t *Element) _string(wr bool) string {
	elementname := t.name
	if t.space != "" {
	...
	if len(t.Attrs) > 0 {
		for _, att := range t.Attrs {
			...
			if wr {
				sattr = fmt.Sprint(sattr, " ", attrname, "=", "\"", XMLFmt(att.Value), "\"")
			} else {
	...
	if len(t.childs) > 0 {
		for _, v := range t.childs {
			el := v.(*Element)
			s = fmt.Sprint(s, el._string(wr))
		}
		if wr {
			return fmt.Sprint(s, XMLFmt(t.Value), "</", elementname, ">", "\n")
		} else {
...
func toStr(t *Element, wr bool) string {
	sattr := ""
	if len(t.Attrs) > 0 {
		for _, att := range t.Attrs {
			attrname := att.name
	...
	if wr {
		return fmt.Sprint("<", t.name, sattr, ">", XMLFmt(t.Value), "</", t.name, ">")
	} else {
...
func (t *Element) MustAttr(name string) string {
	v, ok := t.AttrValue(name)
	if ok {
		return v
	} else {
		panic("设置属性错误，没有参数:" + name)
	}
}
...
func (t *Element) WriteToFile(xmlFileName string) {
	if t._root().isSync {
		t._root().lc.RLock()
		defer t._root().lc.RUnlock()
	} else {
	...
	str := t._root()._string(true)
	str = strings.Replace(str, "><", ">\n<", -1)
	ioutil.WriteFile(xmlFileName, []byte(str), 0755)
}

func XMLFmt(s string) string {
	ss := s
	x := []string{">", "<"}
	y := []string{"&gt;", "&lt;"}
	for i, xx := range x {
		ss = strings.Replace(ss, xx, y[i], -1)
	}
	return ss
}
func LoadByFile(file string) (*Element, error) {
	rmXml, rErr := ioutil.ReadFile(file)
	if rErr != nil {
		return nil, rErr
	}
	return LoadByXml(string(rmXml))
}
